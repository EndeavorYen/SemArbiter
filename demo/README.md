...
`server.py` serves `POST /v1/classifier` and `/v1/systemone`. It scores the supplied options against the supplied evidence and returns a choice aligned by option id. `--mock` returns the first option and is not a model result.

```bash
python demo/server.py --model Qwen/Qwen2.5-3B-Instruct --device cuda --port 8000
```

The driving simulator moved to [JevPilot-Vision](https://github.com/EndeavorYen/JevPilot-Vision).
