...
| **Mapika/decider-2b** | 原生開源決策模型 (Decision-Native Slot Logits) | 2B | 0.792<br/>*(公開紀錄)* | 0.0682<br/>*(凍結日誌)* | 11.1% (4/36)<br/>*(凍結日誌)* | —<br/>*(未下載實測)* | —<br/>*(未下載實測)* | —<br/>*(未下載實測)* | 370.0 MB<br/>*(原生權重)* |
...
| **NanoJev / Qwen3-0.6B** | 微型低功耗決策頭 (Apple M4 邊緣實測) | 0.6B | 0.440 / 0.528<br/>*(凍結實測)* | 0.1420<br/>*(凍結實測)* | 22.2% (8/36)<br/>*(凍結實測)* | —<br/>*(未實測)* | —<br/>*(未在 5080 實測)* | **53.315 ms**<br/>*(實體 M4 MLX 實測)* | 110.0 MB<br/>*(450MB 實測 RAM)* |
...
> 4. **硬體實測物理數據**：
>    - 在本地 **RTX 5080 (Blackwell)** 上實測 CUDA Graphs，短序列 P50 延遲自 **10.138 ms 壓至 5.649 ms**（提速 1.79 倍），P99 尾端抖動降低 51.3%（6.078 ms），顯存搬移量自 741.9 MB 驟降至 20 KB。
>    - 在實體 **Apple Mac mini M4**（`simon@192.168.50.184`）上實測原生 MLX，測得 **53.315 ms P50 延遲** 與 450.4 MB 常駐記憶體，整機功耗約 20W！
...
| **短序列延遲 (L=64)** | **5.649 ms (P50) / 6.078 ms (P99)**<br/>*(未捕獲圖: 10.138 ms P50)* | **53.315 ms (P50) / 53.797 ms (P99)**<br/>*(Qwen2.5-0.5B 4-bit 實測)* |
| **延遲抖動控制** | 消除 51.3% P99 尾端延遲（高度確定性） | UMA 零拷貝，無 PCIe 傳輸波動（抖動僅 0.48ms） |
| **實測常駐 RAM 佔用** | 8.0 GB (BF16 模型空間) | **450.4 MB（實測巔峰記憶體），剩餘 15GB+ 系統空間** |
